...
const mockReferenceData = {
  'departments.csv': [
    { dept_id: 'ENG001', dept_name: 'Engineering', manager: 'Sarah Johnson', budget: 1000000 },
    { dept_id: 'MKT001', dept_name: 'Marketing', manager: 'Mike Chen', budget: 500000 },
    { dept_id: 'HR001', dept_name: 'Human Resources', manager: 'Lisa Wong', budget: 300000 },
    { dept_id: 'FIN001', dept_name: 'Finance', manager: 'David Kim', budget: 750000 },
  ],
  'products.csv': [
...
describe('LookupProcessor', () => {
  ...
  describe('Data Processing with Lookups', () => {
    it('should process data with lookup fields successfully', async () => {
      const result = await processor.processDataWithLookups(sampleData, targetShapeWithLookups);
    ...
    it('should handle data without lookup fields', async () => {
      const result = await processor.processDataWithLookups(sampleData, targetShapeWithoutLookups);
    ...
    it('should collect errors for unmatched values', async () => {
      ...
      const result = await processor.processDataWithLookups(dataWithUnknownValues, targetShapeWithLookups);
    ...
    it('should handle fuzzy matches correctly', async () => {
      ...
      const result = await processor.processDataWithLookups(dataWithTypos, targetShapeWithLookups, {
        minConfidence: 0.6,
      });
  ...
  describe('Single Lookup Processing', () => {
    it('should process a single lookup successfully', async () => {
      const lookupField = targetShapeWithLookups.fields.find(f => f.name === 'department') as LookupField;
      
      const result = await processor.processSingleLookup('Engineering', lookupField, 'test_row');
    ...
    it('should handle missing reference data', async () => {
      ...
      await expect(
        processor.processSingleLookup('test', lookupFieldWithMissingRef, 'test_row')
  ...
  describe('Real-time Lookup Updates', () => {
    it('should process real-time lookup update successfully', async () => {
      ...
      const result = await processor.processLookupUpdate('Marketing', lookupField, rowData);
    ...
    it('should handle failed lookup updates', async () => {
      ...
      const result = await processor.processLookupUpdate('Unknown Department', lookupField, rowData);
  ...
  describe('Error Handling', () => {
    ...
    it('should stop processing on error when continueOnError is false', async () => {
      ...
      await expect(
        processor.processDataWithLookups(malformedData, targetShapeWithLookups, {
  ...
  describe('Performance Testing', () => {
    it('should handle large datasets efficiently', async () => {
      ...
      const result = await processor.processDataWithLookups(largeDataset, targetShapeWithLookups);
  ...
  describe('Integration Scenarios', () => {
    it('should handle mixed exact and fuzzy matches in same dataset', async () => {
      ...
      const result = await processor.processDataWithLookups(mixedData, targetShapeWithLookups);
    ...
    it('should handle multiple derived fields correctly', async () => {
      const result = await processor.processDataWithLookups([sampleData[0]], targetShapeWithLookups);
