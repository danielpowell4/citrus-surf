...
export interface LookupValidator {
  validateLookupField(value: any, field: LookupField, referenceData: any[]): LookupValidationResult;
  generateEnumRules(referenceData: any[], keyColumn: string): ValidationRule[];
  validateReferenceIntegrity(data: any[], keyColumn: string): LookupValidationResult;
  validateConfidenceThreshold(value: any, field: LookupField): Promise<LookupValidationResult>;
}
...
export class LookupValidationSystem implements LookupValidator {
   ...
   */
  async validateLookupField(value: any, field: LookupField, referenceData?: any[]): Promise<LookupValidationResult> {
    const errors: LookupValidationError[] = [];
    const warnings: LookupValidationError[] = [];
    const suggestions: string[] = [];

    // Get reference data if not provided
    const refData = referenceData || referenceDataManager.getReferenceDataRows(field.referenceFile);
    ...
    if (field.smartMatching.enabled && value != null && value !== '') {
      const confidenceValidation = await this.validateConfidenceThreshold(value, field);
    ...
    const integrityValidation = this.validateReferenceIntegrity(refData, field.match.on);
   ...
   */
  private async validateEnumRule(value: any, field: LookupField, referenceData: any[]): Promise<LookupValidationResult> {
    const errors: LookupValidationError[] = [];
    const warnings: LookupValidationError[] = [];
    const suggestions: string[] = [];
    ...
    const exactMatch = availableOptions.some(option => 
      String(option).toLowerCase() === stringValue.toLowerCase()
    );

    if (!exactMatch) {
      // If no exact match, try to find similar options for suggestions
      const similarOptions = this.findSimilarOptions(stringValue, availableOptions, 0.6);
   ...
   */
  async validateConfidenceThreshold(value: any, field: LookupField): Promise<LookupValidationResult> {
    const errors: LookupValidationError[] = [];
    const warnings: LookupValidationError[] = [];
    const suggestions: string[] = [];
    ...
    try {
      ...
      const result = await lookupProcessor.processSingleLookup(value, field, 'validation_row');
  ...
  validateReferenceIntegrity(data: any[], keyColumn: string): LookupValidationResult {
    ...
    if (!sampleRow || !sampleRow.hasOwnProperty(keyColumn)) {
      ...
      errors.push({
        ...
        suggestions: availableColumns.filter(key => 
   ...
   */
  private findSimilarOptions(input: string, options: string[], threshold: number = 0.6): string[] {
    ...
    for (const option of options) {
      ...
      // Starts with
      else if (optionLower.startsWith(inputLower) || inputLower.startsWith(optionLower)) {
        score = 0.8;
      }
      // Contains
  ...
  getValidationStats(results: Array<{ result: LookupValidationResult }>): {
    ...
    const errorCount = results.reduce((sum, r) => sum + r.result.errors.length, 0);
...
export function generateLookupValidation(field: LookupField): ValidationRule[] {
  try {
    ...
    return lookupValidationSystem.generateEnumRules(referenceData, field.match.on);
  } catch (error) {
...
export function isLookupValidationRule(rule: ValidationRule): boolean {
  return ['lookup_enum', 'lookup_confidence', 'lookup_reference'].includes(rule.type);
}

/**
 * Utility function to extract suggestions from validation errors
 */
export function extractSuggestionsFromErrors(errors: LookupValidationError[]): string[] {
  return errors
