...
function validateMapping(mapping: Record<string, string>, targetFields: TargetField[]): {
...
function applyMapping(data: any[], mapping: Record<string, string>, targetFields: TargetField[]): any[] {
  return data.map(row => {
...
describe('Column Mapping Logic', () => {
  ...
  it('handles empty mappings gracefully', () => {
    const emptyMapping = {};
    const transformedData = applyMapping(mockData, emptyMapping, mockTargetFields);
  ...
  it('suggests mappings for complex field name patterns', () => {
    const complexColumns = ['user_id', 'full_name', 'email_address', 'department_name'];
