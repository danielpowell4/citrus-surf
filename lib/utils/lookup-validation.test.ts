...
vi.mock("./lookup-processor", () => ({
  lookupProcessor: {
    processSingleLookup: vi.fn(async (value: any, field: any) => {
      const referenceData =
        mockReferenceData[
          field.referenceFile as keyof typeof mockReferenceData
        ];
      if (!referenceData) {
      ...
      const exactMatch = referenceData.find(
        row =>
...
describe("LookupValidationSystem", () => {
  ...
  describe("Enum Validation", () => {
    it("should validate exact matches successfully", async () => {
      const result = await validator.validateLookupField(
        "Engineering",
        departmentLookupField
      );
    ...
    it("should handle empty/null values gracefully", async () => {
      const result1 = await validator.validateLookupField(
        "",
        departmentLookupField
      );
      const result2 = await validator.validateLookupField(
        null,
        departmentLookupField
      );
      const result3 = await validator.validateLookupField(
        undefined,
        departmentLookupField
      );
  ...
  describe("Fuzzy Match Confidence Validation", () => {
    ...
    it("should warn about low confidence matches", async () => {
      ...
      const hasConfidenceIssue =
        result.errors.some(e => e.type === "lookup_confidence") ||
    ...
    it("should provide confidence scores in validation results", async () => {
      ...
      const confidenceError =
        result.errors.find(e => e.type === "lookup_confidence") ||
  ...
  describe("Reference Data Integrity Validation", () => {
    ...
    it("should detect duplicate keys", () => {
      ...
      expect(
        result.warnings.some(w => w.message.includes("Duplicate values"))
      ).toBe(true);
    });
    ...
    it("should detect empty/null keys", () => {
      ...
      expect(
        result.warnings.some(w => w.message.includes("empty values"))
      ).toBe(true);
    });
  ...
  describe("Enhanced Error Messages", () => {
    ...
    it('should provide "Did you mean?" suggestions', async () => {
      ...
      expect(result.suggestions.length).toBeGreaterThan(0);
      expect(
        result.suggestions.some(s => s.toLowerCase().includes("marketing"))
      ).toBe(true);
    });
