...
describe('Lookup Navigation Utilities', () => {
  describe('URL Generation', () => {
    ...
    it('generates lookup config URLs with field and action parameters', () => {
      const url = generateLookupConfigUrl({ field: 'field_123', action: 'configure-lookup' });
      expect(url).toBe('/playground/template-builder?field=field_123&action=configure-lookup');
    });

    it('generates fuzzy match review URLs with parameters', () => {
      const url = generateFuzzyMatchReviewUrl({ review: 'fuzzy-matches', batch: 'batch_123' });
      expect(url).toBe('/playground/data-table?review=fuzzy-matches&batch=batch_123');
    });
  });
  ...
  describe('URL Parsing', () => {
    ...
    it('parses lookup config parameters from URLSearchParams', () => {
      const searchParams = new URLSearchParams('field=field_123&action=configure-lookup');
    ...
    it('parses fuzzy match review parameters from URLSearchParams', () => {
      const searchParams = new URLSearchParams('review=fuzzy-matches&batch=batch_123');
  ...
  describe('LookupNavigator', () => {
    ...
    it('navigates to reference data viewer', () => {
      ...
      expect(mockRouter.push).toHaveBeenCalledWith('/playground/reference-data?file=ref_123&mode=view');
    });

    it('navigates to reference data editor', () => {
      navigator.toReferenceDataEditor('ref_123');
      
      expect(mockRouter.push).toHaveBeenCalledWith('/playground/reference-data?file=ref_123&mode=edit');
    });

    it('navigates to lookup configuration', () => {
      navigator.toLookupConfiguration('field_123');
      
      expect(mockRouter.push).toHaveBeenCalledWith('/playground/template-builder?field=field_123&action=configure-lookup');
    });

    it('navigates to fuzzy match review', () => {
      navigator.toFuzzyMatchReview('batch_123');
      
      expect(mockRouter.push).toHaveBeenCalledWith('/playground/data-table?review=fuzzy-matches&batch=batch_123');
    });

    it('uses replace when specified', () => {
      navigator.toReferenceDataViewer('ref_123', { replace: true });
      
      expect(mockRouter.replace).toHaveBeenCalledWith('/playground/reference-data?file=ref_123&mode=view');
    ...
    it('navigates to template builder', () => {
      ...
      expect(mockRouter.push).toHaveBeenCalledWith('/playground/template-builder');
    });

    it('navigates to reference data management', () => {
      navigator.toReferenceDataManagement();
      
      expect(mockRouter.push).toHaveBeenCalledWith('/playground/reference-data');
    });
  });
  ...
  describe('ID Validation', () => {
    it('validates reference IDs', () => {
      ...
      expect(isValidReferenceId('batch_01H9X2K3L4M5N6P7Q8R9S0T1U2')).toBe(false);
    });
  ...
  describe('Breadcrumb Generation', () => {
    it('generates breadcrumbs for reference data root', () => {
      const breadcrumbs = generateLookupBreadcrumbs('/playground/reference-data', new URLSearchParams(''));
      
      expect(breadcrumbs).toEqual([
        { label: 'Playground', href: '/playground' },
        { label: 'Reference Data', href: '/playground/reference-data', current: true },
      ]);
    });
    ...
    it('generates breadcrumbs for reference data viewer', () => {
      ...
      expect(breadcrumbs).toEqual([
        ...
        { label: 'View File', href: '/playground/reference-data?file=ref_123&mode=view', current: true },
      ]);
    });
    ...
    it('generates breadcrumbs for reference data editor', () => {
      ...
      expect(breadcrumbs).toEqual([
        { label: 'Playground', href: '/playground' },
        { label: 'Reference Data', href: '/playground/reference-data' },
        { label: 'Edit File', href: '/playground/reference-data?file=ref_123&mode=edit', current: true },
      ]);
    });
    ...
    it('generates breadcrumbs for template builder with lookup config', () => {
      ...
      expect(breadcrumbs).toEqual([
        ...
        { label: 'Configure Lookup', href: '/playground/template-builder?field=field_123&action=configure-lookup', current: true },
      ]);
    });

    it('generates breadcrumbs for fuzzy match review', () => {
      const searchParams = new URLSearchParams('review=fuzzy-matches&batch=batch_123');
      const breadcrumbs = generateLookupBreadcrumbs('/playground/data-table', searchParams);
      
      expect(breadcrumbs).toEqual([
        { label: 'Playground', href: '/playground' },
        { label: 'Data Table', href: '/playground/data-table' },
        { label: 'Review Fuzzy Matches', href: '/playground/data-table?review=fuzzy-matches&batch=batch_123', current: true },
      ]);
    });
  });
  ...
  describe('Route Detection', () => {
    it('detects reference data routes', () => {
      expect(isLookupRoute('/playground/reference-data', new URLSearchParams(''))).toBe(true);
      expect(isLookupRoute('/playground/reference-data', new URLSearchParams('file=ref_123'))).toBe(true);
    });

    it('detects lookup configuration routes', () => {
      const searchParams = new URLSearchParams('action=configure-lookup');
      expect(isLookupRoute('/playground/template-builder', searchParams)).toBe(true);
      expect(isLookupRoute('/playground/template-builder', new URLSearchParams(''))).toBe(false);
    });
    ...
    it('detects fuzzy match review routes', () => {
      ...
      expect(isLookupRoute('/playground/data-table', new URLSearchParams(''))).toBe(false);
    });

    it('does not detect non-lookup routes', () => {
      expect(isLookupRoute('/playground', new URLSearchParams(''))).toBe(false);
      expect(isLookupRoute('/tools/csv-to-json', new URLSearchParams(''))).toBe(false);
    });
  });
  ...
  describe('Suggested Next Actions', () => {
    it('suggests actions for reference data edit completion', () => {
      const searchParams = new URLSearchParams('file=ref_123&mode=edit');
      const actions = getSuggestedNextActions('/playground/reference-data', searchParams, {
    ...
    it('returns empty actions for unknown contexts', () => {
      const actions = getSuggestedNextActions('/unknown/route', new URLSearchParams(''));
