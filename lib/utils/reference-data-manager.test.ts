...
describe("ReferenceDataManager", () => {
  ...
  describe("File Upload and Parsing", () => {
    ...
    it("should upload and parse a JSON file", async () => {
      ...
      const file = new File([jsonContent], "departments.json", {
    ...
    it("should handle CSV with tab delimiter", async () => {
      const csvContent =
        "name\tage\tdepartment\nJohn\t30\tEngineering\nJane\t25\tMarketing";
      const file = new File([csvContent], "employees.tsv", {
    ...
    it("should reject duplicate IDs without overwrite option", async () => {
      ...
      await expect(manager.uploadReferenceFile(file, id)).rejects.toThrow(
        "already exists"
      );
    });
  ...
  describe("Data Validation", () => {
    ...
    it("should detect invalid JSON", async () => {
      ...
      expect(result.errors.some(error => error.includes("Invalid JSON"))).toBe(
        true
      );
    });
    ...
    it("should warn about large files", async () => {
      ...
      const largeContent =
        "name,age,department,description\n" +
      ...
      expect(
        result.warnings.some(warning => warning.includes("Large file size"))
      ).toBe(true);
    });
    ...
    it("should reject files that are too large", async () => {
      ...
      manager["readFileContent"] = vi
        .fn()
        .mockResolvedValue("name,age\nJohn,30");
  ...
  describe("Data Retrieval and Management", () => {
    ...
    it("should update existing reference data", async () => {
      ...
      const updated = manager.updateReferenceData(
        "ref_test_employees",
        newData
      );
