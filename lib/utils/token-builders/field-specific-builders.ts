...
export class EmailTokenBuilder extends BaseTokenBuilder {
  ...
  generateTokens(context: TokenContext): TokenResult {
    ...
    primaryTokens.forEach(token => {
      this.generateCaseVariations(token).forEach(variation =>
        tokens.add(variation)
      );
    });
...
export class PhoneTokenBuilder extends BaseTokenBuilder {
  ...
  canHandle(context: TokenContext): boolean {
    return (
      super.canHandle(context) ||
      this.containsKeywords(context.fieldName, [
        "phone",
        "tel",
        "mobile",
        "cell",
...
export class NameTokenBuilder extends BaseTokenBuilder {
  ...
  canHandle(context: TokenContext): boolean {
    return (
      this.containsKeywords(context.fieldName, ["name"]) ||
  ...
  generateTokens(context: TokenContext): TokenResult {
    ...
    if (
      this.containsKeywords(context.fieldName, ["first"]) ||
    ...
    if (
      this.containsKeywords(context.fieldName, ["last", "sur"]) ||
    ...
    if (
      this.containsKeywords(context.fieldName, ["full"]) ||
...
export class IdTokenBuilder extends BaseTokenBuilder {
  ...
  canHandle(context: TokenContext): boolean {
    return (
      this.containsKeywords(context.fieldName, ["id"]) ||
  ...
  generateTokens(context: TokenContext): TokenResult {
    ...
    if (
      fieldLower.includes("customer") ||
      fieldIdLower.includes("customer") ||
      fieldLower.includes("cust") ||
      fieldIdLower.includes("cust")
    ) {
...
export class DateTimeTokenBuilder extends BaseTokenBuilder {
  ...
  canHandle(context: TokenContext): boolean {
    return (
      ...
      this.containsKeywords(context.fieldId, [
        "date",
        "time",
        "created",
        "updated",
  ...
  generateTokens(context: TokenContext): TokenResult {
    ...
    if (
      this.containsKeywords(context.fieldName, ["created"]) ||
    ...
    if (
      this.containsKeywords(context.fieldName, ["updated", "modified"]) ||
    ...
    if (
      this.containsKeywords(context.fieldName, ["birth", "born"]) ||
...
export class NumericTokenBuilder extends BaseTokenBuilder {
  ...
  generateTokens(context: TokenContext): TokenResult {
    ...
    // Age variations
    if (
      this.containsKeywords(context.fieldName, ["age"]) ||
    ...
    // Count variations
    if (
      this.containsKeywords(context.fieldName, ["count", "total", "number"]) ||
...
export class AddressTokenBuilder extends BaseTokenBuilder {
  ...
  generateTokens(context: TokenContext): TokenResult {
    ...
    // Address variations
    if (
      this.containsKeywords(context.fieldName, ["address"]) ||
    ...
    // Street variations
    if (
      this.containsKeywords(context.fieldName, ["street"]) ||
    ...
    // City variations
    if (
      this.containsKeywords(context.fieldName, ["city"]) ||
    ...
    if (
      this.containsKeywords(context.fieldName, ["state", "province"]) ||
    ...
    if (
      this.containsKeywords(context.fieldName, ["zip", "postal"]) ||
    ...
    // Country variations
    if (
      this.containsKeywords(context.fieldName, ["country"]) ||
