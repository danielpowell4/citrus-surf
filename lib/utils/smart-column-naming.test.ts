...
import { generateSmartColumnName, generateSmartDescription, generateLookupPreview } from './smart-column-naming';
...
describe('Smart Column Naming', () => {
  describe('generateSmartColumnName', () => {
    it('should generate contextual names for derived fields', () => {
      ...
      const result = generateSmartColumnName(lookupField, derivedField, 'manager');
      expect(result).toBe('department_manager');
    });

    it('should use existing name if provided', () => {
      const lookupField = 'department';
      const derivedField: DerivedField = { name: 'dept_head', source: 'manager' };
    ...
    it('should handle cases where source already contains context', () => {
      ...
      const result = generateSmartColumnName(lookupField, derivedField, 'product_price');
  ...
  describe('generateSmartDescription', () => {
    ...
    it('should include reference file name when provided', () => {
      const lookupField = 'department';
      const derivedField: DerivedField = { name: 'unknown_field', source: 'unknown_field' };
      
      const result = generateSmartDescription(lookupField, derivedField, 'departments.csv');
    ...
    it('should correctly infer types from sample values', () => {
      const lookupField: LookupField = {
        ...
        alsoGet: [
          ...
          { name: '', source: 'fake_date_string', targetFieldName: 'fake_date_string' },
        ],
