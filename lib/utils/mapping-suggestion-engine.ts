...
function findBestMatch(
  ...
  const fieldVariations = generateFieldVariations(
    targetField.name,
    targetField.id,
    targetField.type
  );
  ...
  for (const column of importColumns) {
    ...
    for (const fieldVar of fieldVariations) {
      for (const colVar of columnVariations) {
        if (
          fieldVar === colVar &&
          fieldVar !== targetField.name.toLowerCase() &&
          fieldVar !== targetField.id.toLowerCase()
        ) {
  ...
  for (const column of importColumns) {
    ...
    if (bestConfidence >= fuzzyThreshold) {
      ...
      if (
        !bestFuzzyMatch ||
        suggestion.confidence > bestFuzzyMatch.confidence
      ) {
