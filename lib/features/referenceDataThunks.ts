...
export const deleteReferenceFile = createAsyncThunk<
  ...
  { id: string },
  { state: RootState }
>("referenceData/deleteFile", async ({ id }, { dispatch, rejectWithValue }) => {
  try {
...
export const clearAllReferenceData = createAsyncThunk<
  ...
  void,
  { state: RootState }
>("referenceData/clearAll", async (_, { dispatch }) => {
...
export const restoreReferenceDataToIndex = createAsyncThunk<
  ...
  { index: number },
  { state: RootState }
>("referenceData/restoreToIndex", async ({ index }, { dispatch, getState }) => {
      ...
      switch (action.type) {
        ...
        case "update":
          ...
          if (action.data?.previousData) {
            referenceDataManager.updateReferenceData(
              action.id,
              action.data.previousData
            );
          }
          break;
...
export const undoReferenceDataAction = createAsyncThunk<
  ...
  void,
  { state: RootState }
>("referenceData/undo", async (_, { dispatch, getState }) => {
  const state = getState();
  const { currentHistoryIndex } = state.referenceData;

  if (currentHistoryIndex > 0) {
    await dispatch(
      restoreReferenceDataToIndex({ index: currentHistoryIndex - 1 })
    );
  }
});
...
export const redoReferenceDataAction = createAsyncThunk<
  ...
  void,
  { state: RootState }
>("referenceData/redo", async (_, { dispatch, getState }) => {
  const state = getState();
...
export const initializeReferenceData = createAsyncThunk<
  ...
  void,
  { state: RootState }
>("referenceData/initialize", async (_, { dispatch }) => {
