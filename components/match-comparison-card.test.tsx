...
describe("MatchComparisonCard", () => {
  ...
  it("shows manual entry input when manual button is clicked", () => {
    ...
    expect(
      screen.getByPlaceholderText("Enter correct value...")
    ).toBeInTheDocument();
    // Check for buttons with check and X icons
    const buttons = screen.getAllByRole("button");
    const hasCheckButton = buttons.some(button =>
      button.querySelector('svg[class*="lucide-check"]')
    );
    const hasXButton = buttons.some(button =>
  ...
  it("cancels manual entry with Escape key", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByPlaceholderText("Enter correct value...")
  ...
  it("shows manual entry status", () => {
    ...
    expect(
      screen.getByText("Manual entry: Custom Department")
    ).toBeInTheDocument();
  });
  ...
  it("applies correct confidence color classes", () => {
    ...
    const { rerender } = render(
      <MatchComparisonCard {...mockProps} match={highConfidenceMatch} />
    );

    expect(screen.getByText("90% match")).toHaveClass(
      "bg-green-100",
      "text-green-800"
    );
    ...
    rerender(
      <MatchComparisonCard {...mockProps} match={mediumConfidenceMatch} />
    );

    expect(screen.getByText("70% match")).toHaveClass(
      "bg-yellow-100",
      "text-yellow-800"
    );
    ...
    expect(screen.getByText("50% match")).toHaveClass(
      "bg-red-100",
      "text-red-800"
    );
  });
  ...
  it("applies status-specific styling", () => {
    ...
    const acceptedMatch = { ...mockMatch, status: "accepted" as const };
    const { rerender } = render(
      <MatchComparisonCard {...mockProps} match={acceptedMatch} />
    );

    let card = screen
      .getByText("Accepted: Engineering")
    ...
    card = screen
      .getByText("Rejected - no match applied")
      .closest(".border-red-200");
