...
import {
  ...
  type DemoTemplate,
} from "@/lib/utils/demo-reference-templates";
...
export function DemoTemplateSelector({
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<DemoTemplate | null>(
    null
  );
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose from pre-built reference data that's perfect for demos and
            testing. Each template includes realistic data and suggested lookup
            configurations.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2">
          ...
          <div className="bg-muted/30 rounded p-2 text-xs">
            ...
            <div className="font-mono text-muted-foreground mt-1">
              {Object.values(template.data[0])
                .slice(0, 4)
                .map(v => String(v).slice(0, 12))
                .join(" | ")}
              {Object.keys(template.data[0]).length > 4 && " | ..."}
            </div>
          </div>
          ...
          <div className="bg-blue-50 dark:bg-blue-950/50 border border-blue-200 dark:border-blue-800 rounded p-2">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="font-medium text-foreground">
                {template.suggestedLookups[0].matchOn}
              </span>
              <ArrowRight className="h-3 w-3 text-muted-foreground" />
              <span className="font-medium text-foreground">
                {template.suggestedLookups[0].returnField}
              </span>
            </div>
        ...
        {sampleInput.length > 0 && (
          <div className="space-y-2">
            ...
            <div className="bg-green-50 dark:bg-green-950/50 border border-green-200 dark:border-green-800 rounded p-2 text-xs text-foreground">
              Sample input data with {Object.keys(sampleInput[0]).join(", ")}{" "}
              columns
            </div>
          </div>
        )}
