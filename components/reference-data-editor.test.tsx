...
describe('ReferenceDataEditor', () => {
  ...
  it('allows deleting rows', async () => {
    ...
    const deleteButton = deleteButtons.find(button => button.innerHTML.includes('Trash2'));
  ...
  it('validates required key column values', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Validation Errors:')).toBeInTheDocument();
      expect(screen.getByText(/key column cannot be empty/i)).toBeInTheDocument();
    });
  });
  ...
  it('disables save button when validation errors exist', async () => {
    ...
    await waitFor(() => {
      const saveButton = screen.getByRole('button', { name: /fix errors first/i });
  ...
  it('shows row count and editing instructions', () => {
    ...
    expect(screen.getByText('3 rows • Click any cell to edit')).toBeInTheDocument();
  });
