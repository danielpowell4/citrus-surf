...
describe("ReferenceDataViewer", () => {
  ...
  beforeEach(() => {
    ...
    mockReferenceDataManager.getReferenceDataRows.mockReturnValue(
      mockReferenceData
    );
  ...
  it("shows impact analysis for affected lookup fields", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/This reference data is used by 1 lookup field/)
      ).toBeInTheDocument();
  ...
  it("shows edit button when allowEdit is true", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: /edit data/i })
      ).toBeInTheDocument();
    });
  });
  ...
  it("does not show edit button when allowEdit is false", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByRole("button", { name: /edit data/i })
