...
export function DevClaudeHelper() {
  ...
  const formatElementContextForClaude = useCallback((context: any): string => {
    ...
    const importantStyles = ['display', 'position', 'width', 'height', 'margin', 'padding', 'border', 'background', 'color', 'font-size', 'z-index'];
  ...
  const extractElementContext = useCallback((element: HTMLElement) => {
    const context = {
      element: {
  ...
  const copyElementForClaude = () => {
    ...
    if (globalLogger && globalLogger.copyElementForClaude && selectedElement) {
      globalLogger.copyElementForClaude();
      // Show success feedback
      const button = document.querySelector('[data-copy-element-button]') as HTMLElement;
      if (button) {
        const originalText = button.textContent;
        button.textContent = 'Copied!';
        setTimeout(() => {
  ...
  return (
    <>
      {/* Floating Button */}
      <div className={`fixed bottom-4 left-1/2 transform -translate-x-1/2 ${isSelectMode ? 'z-[9994]' : 'z-[9999]'}`} data-claude-helper>
        <Button
      ...
      {/* Claude Helper Panel */}
      {isOpen && (
        <div className={`fixed bottom-16 left-1/2 transform -translate-x-1/2 w-96 max-w-[90vw] ${isSelectMode ? 'z-[9995]' : 'z-[9998]'}`} data-claude-helper>
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm flex items-center gap-2">
                  ...
                  Claude Helper
                  {errorCount > 0 && (
                    <Badge variant="secondary" className="text-xs bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200">
                      {errorCount} errors
                    </Badge>
                  )}
                </CardTitle>
                ...
                {selectedElement && (
                  <div className="mt-3 p-2 bg-green-50 dark:bg-green-950 rounded text-xs">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-medium text-green-800 dark:text-green-200">Element Selected</span>
                    </div>
                ...
                {isSelectMode && (
                  <div className="mt-3 p-2 bg-orange-50 dark:bg-orange-950 rounded text-xs text-orange-800 dark:text-orange-200">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-medium">Element Selection Mode Active</span>
                    </div>
                    <div>
                      Hover over elements and click to select • Press Escape to exit
                    </div>
                  </div>
                )}
      ...
      {isSelectMode && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-[9999] pointer-events-none">
          <div className="bg-blue-500 text-white px-4 py-2 rounded-full shadow-lg flex items-center gap-2">
            ...
            <span className="text-sm font-medium">Element Selection Mode - Click any element</span>
          </div>
        </div>
      )}
