...
describe("ReferenceUploadDialog", () => {
  ...
  beforeEach(() => {
    ...
    global.File.prototype.text = vi.fn().mockImplementation(async function (
      this: File
    ) {
  ...
  it("renders replace dialog when in replace mode", () => {
    ...
    expect(
      screen.getByText("Replacing: existing-data.csv")
    ).toBeInTheDocument();
  });
  ...
  it("shows file drop zone", () => {
    ...
    expect(
      screen.getByText("Supports CSV and JSON files up to 10MB")
    ).toBeInTheDocument();
  });
  ...
  it("shows custom ID input in upload mode", () => {
    ...
    expect(
      screen.getByLabelText("Reference ID (optional)")
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText("Auto-generated if empty")
    ).toBeInTheDocument();
  });
  ...
  it("does not show custom ID input in replace mode", () => {
    ...
    expect(
      screen.queryByLabelText("Reference ID (optional)")
    ).not.toBeInTheDocument();
  });

  it("handles file selection via input", async () => {
    // Set up successful validation for this test
    mockReferenceDataManager.validateReferenceData.mockResolvedValue(
    ...
    await waitFor(() => {
      ...
      expect(
        mockReferenceDataManager.validateReferenceData
  ...
  it("auto-generates custom ID from filename", async () => {
    ...
    await waitFor(() => {
      const customIdInput = screen.getByLabelText(
  ...
  it("displays file validation warnings", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("File validation passed")).toBeInTheDocument();
      expect(
        screen.getByText("Some rows have empty values")
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows upload progress to user", async () => {
    ...
    mockReferenceDataManager.uploadReferenceFile.mockImplementation(
      async () => {
  ...
  it("completes file upload successfully", async () => {
    ...
    mockReferenceDataManager.uploadReferenceFile.mockResolvedValue(
      mockReferenceInfo
    );
  ...
  it("provides error feedback when upload fails", async () => {
    ...
    mockReferenceDataManager.uploadReferenceFile.mockRejectedValue(
