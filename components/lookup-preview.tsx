...
import {
  ...
  type LookupPreview as PreviewData,
...
export function LookupPreview({
  ...
  return (
    <Card
      className={`border-blue-200 dark:border-blue-800 bg-blue-50/30 dark:bg-blue-950/30 ${className}`}
    >
      ...
      <CardContent className="space-y-3">
        ...
        <div className="text-xs text-muted-foreground bg-white/50 dark:bg-slate-800/50 p-2 rounded border border-border">
          <span className="font-medium">Result:</span> Your data will be
          enriched with{" "}
  ...
  return (
    <div className={`flex items-center gap-2 text-xs ${className}`}>
      ...
      <span className="text-muted-foreground">
        Will add {preview.derivedColumns.length} column
        {preview.derivedColumns.length !== 1 ? "s" : ""}:
      </span>
