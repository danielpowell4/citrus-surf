...
describe('useFuzzyMatchReview', () => {
  ...
  it('accepts a match correctly', () => {
    ...
    const { result } = renderHook(() => useFuzzyMatchReview(mockMatches, onMatchUpdated));
    ...
    // Check state updates
    const acceptedMatch = result.current.state.matches.find(m => m.id === matchId);
    ...
    expect(onMatchUpdated).toHaveBeenCalledWith(matchId, 'accepted', 'Engineering');
  ...
  it('rejects a match correctly', () => {
    ...
    // Check state updates
    const rejectedMatch = result.current.state.matches.find(m => m.id === matchId);
  ...
  it('sets manual value correctly', () => {
    ...
    // Check state updates
    const manualMatch = result.current.state.matches.find(m => m.id === matchId);
    ...
    // Check callback was called
    expect(onMatchUpdated).toHaveBeenCalledWith(matchId, 'manual', 'Custom Department');
  ...
  it('accepts selected matches in batch', () => {
    ...
    const acceptedMatches = result.current.state.matches.filter(m => m.status === 'accepted');
    expect(acceptedMatches).toHaveLength(2);
    expect(acceptedMatches.every(m => m.manualValue === 'Batch Value')).toBe(true);
  ...
  it('rejects selected matches in batch', () => {
    ...
    const rejectedMatches = result.current.state.matches.filter(m => m.status === 'rejected');
  ...
  it('filters by search term', () => {
    ...
    expect(result.current.state.filteredMatches[0].suggestedValue).toBe('Engineering');
  });
  ...
  it('resets all matches correctly', () => {
    ...
    expect(result.current.hasChanges).toBe(false);
    expect(result.current.state.selectedMatches.size).toBe(0);
    expect(result.current.state.matches.every(m => m.status === 'pending')).toBe(true);
    expect(result.current.state.stats.pending).toBe(3);
    expect(result.current.state.stats.accepted).toBe(0);
  ...
  it('sorts filtered matches by confidence and field name', () => {
    ...
    expect(filtered[0].confidence).toBe(0.90); // Highest confidence first
