 ...
 */
function enhanceFuzzyMatch(match: FuzzyMatch, index: number): FuzzyMatchForReview {
  return {
    ...match,
    ...
    suggestions: match.confidence < 0.8 ? [
      {
...
function calculateStats(matches: FuzzyMatchForReview[]): FuzzyMatchReviewStats {
  ...
  const progress = total > 0 ? Math.round(((accepted + rejected + manual) / total) * 100) : 0;
 ...
 */
function applyFilter(matches: FuzzyMatchForReview[], filter: FuzzyMatchFilter): FuzzyMatchForReview[] {
  ...
  if (filter.searchTerm) {
    ...
    filtered = filtered.filter(m => 
      ...
      (m.suggestions || []).some(s => String(s.value).toLowerCase().includes(searchLower))
    );
  }
...
export function useFuzzyMatchReview(
  ...
  onMatchUpdated?: (matchId: string, status: FuzzyMatchForReview['status'], value?: any) => void
): UseFuzzyMatchReviewResult {
  ...
  const filteredMatches = useMemo(() => applyFilter(matches, filter), [matches, filter]);
  const hasChanges = useMemo(() => matches.some(m => m.status !== 'pending'), [matches]);
  const isComplete = useMemo(() => matches.every(m => m.status !== 'pending'), [matches]);
  const canBatchOperate = selectedMatches.size > 0;

  // Actions
  const acceptMatch = useCallback((matchId: string, acceptedValue?: any) => {
    setMatches(prev => prev.map(match => 
      match.id === matchId 
        ? { ...match, status: 'accepted' as const, manualValue: acceptedValue || match.suggestedValue }
        : match
    ));
    ...
    dispatch({
      ...
      payload: { matchId, acceptedValue: acceptedValue || matches.find(m => m.id === matchId)?.suggestedValue }
    });
  ...
  const setManualValue = useCallback((matchId: string, value: any) => {
    setMatches(prev => prev.map(match => 
      match.id === matchId 
  ...
  const selectAll = useCallback((criteria?: Partial<FuzzyMatchFilter>) => {
    const targetMatches = criteria ? applyFilter(matches, criteria) : filteredMatches;
  ...
  const acceptSelected = useCallback((acceptedValue?: any) => {
    ...
    selectedIds.forEach(matchId => {
      ...
      onMatchUpdated?.(matchId, 'accepted', acceptedValue || match?.suggestedValue);
    });
  ...
  const updateFilter = useCallback((newFilter: Partial<FuzzyMatchFilter>) => {
  ...
  const resetAll = useCallback(() => {
    setMatches(prev => prev.map(match => ({ ...match, status: 'pending' as const, manualValue: undefined })));
    clearSelection();
  }, [clearSelection]);
