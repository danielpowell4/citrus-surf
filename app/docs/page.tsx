...
function getDocCategory(filename: string): string {
  ...
  if (filename.includes('lookup') || filename.includes('reference-data')) return 'Lookup System';
  if (filename.includes('history') || filename.includes('persistence')) return 'State Management';
  if (filename.includes('import') || filename.includes('export')) return 'Data Processing';
  if (filename.includes('style') || filename.includes('component')) return 'UI/UX';
  if (filename.includes('test') || filename.includes('error')) return 'Development';
  return 'General';
}

function getDocPriority(filename: string): 'high' | 'medium' | 'low' {
  const highPriority = ['README.md', 'target-shapes.md', 'import-system.md', 'style-guide.md'];
  const mediumPriority = ['export-system.md', 'history-system.md', 'editable-cells.md'];
                        ...
                        <Card className="h-full hover:shadow-lg transition-shadow cursor-pointer border border-gray-200 dark:border-gray-700">
                          <CardHeader className="pb-3">
                            ...
                            <CardTitle className="text-lg leading-tight">
                              {doc.name.replace('.md', '').replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                            </CardTitle>
                          </CardHeader>
                          <CardContent>
                            ...
                            <div className="text-sm text-gray-500 dark:text-gray-400">
                              ...
                              {doc.lastModified && (
                                <span className="ml-2">
                                  Updated {doc.lastModified.toLocaleDateString()}
                                </span>
                              )}
                            </div>
                          </CardContent>
                          ...
                          <Card className="h-full hover:shadow-lg transition-shadow cursor-pointer border border-gray-200 dark:border-gray-700">
                            ...
                            <CardContent>
                              <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
                                {subItems.length} document{subItems.length !== 1 ? 's' : ''}
                              </p>
                              <div className="text-xs text-gray-500 dark:text-gray-400">
                                {subItems.slice(0, 3).map(item => item.name.replace('.md', '')).join(', ')}
                                {subItems.length > 3 && '...'}
                              </div>
                            </CardContent>
                          </Card>
