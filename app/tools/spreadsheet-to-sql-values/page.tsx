...
function SqlConverterExplanation() {
  return (
    ...
    >
      ...
      <p>
        A Spreadsheet to SQL converter is a tool that transforms data from various spreadsheet formats including CSV
        files, Google Spreadsheets exports, and Excel spreadsheets into SQL statements that can be executed in a
        database. This tool bridges the gap between spreadsheet data and database operations, making it easy to import
        or update data in your database regardless of whether your data comes from CSV files, Google Spreadsheets, or
        Excel workbooks.
      </p>
      ...
      <ul>
        <li>
          <strong>Data Migration:</strong> Quickly convert exported data from Google Spreadsheets, Excel, or CSV files
          from one system into SQL for importing into another database.
        </li>
        <li>
          <strong>Bulk Updates:</strong> Generate UPDATE statements to modify multiple records at once using data from
          CSV exports, Google Spreadsheets, or Excel files.
        </li>
        <li>
          <strong>Database Seeding:</strong> Create SQL statements to populate test or development databases with sample
          data from spreadsheet applications.
        </li>
        <li>
          <strong>Data Correction:</strong> Generate SQL to fix data issues across multiple records using data exported
          from Google Spreadsheets, Excel, or saved as CSV files.
        </li>
      </ul>

      <h3>How to Use This Tool</h3>
      <ol>
        <li>Export your data from Google Spreadsheets, Excel, or save as CSV format</li>
        ...
        <li>Copy the generated SQL and execute it in your database management tool</li>
      </ol>

      <h3>Working with Different Spreadsheet Formats</h3>
      <ul>
        <li>
          <strong>Google Spreadsheets:</strong> Copy and paste directly from Google Sheets, or download as CSV/TSV and
          paste the contents.
        </li>
        <li>
          <strong>Excel:</strong> Copy data from Excel and paste directly, or save as CSV/Tab-delimited text and paste
          the file contents.
        </li>
        <li>
          <strong>CSV Files:</strong> Open CSV files in any text editor and copy the contents, or import into Google
          Spreadsheets/Excel first.
        </li>
      </ul>

      <h3>Understanding UPDATE Loops</h3>
      <p>
        The UPDATE loop feature generates a PostgreSQL DO block that iterates through your spreadsheet data and updates
        records one by one. This approach is particularly useful when:
      </p>
      <ul>
        <li>You need to update multiple columns across many rows from your Google Spreadsheets or Excel data</li>
      ...
      <ul>
        <li>
          <strong>Include Headers:</strong> For UPDATE loops, include column headers in your first row when copying from
          Google Spreadsheets, Excel, or CSV files to enable automatic column mapping.
        </li>
        <li>
          <strong>Check Data Types:</strong> Ensure your spreadsheet data types match what's expected in your database,
          especially for UUIDs and dates.
        </li>
        <li>
          <strong>Test First:</strong> Always test your generated SQL on a small dataset or test database before running
          it on production data.
        </li>
        <li>
          <strong>Backup:</strong> Create a database backup before executing large update operations from spreadsheet
          data.
        </li>
        <li>
          <strong>Format Consistency:</strong> When working with Google Spreadsheets or Excel, ensure consistent
          formatting before copying data.
        </li>
      </ul>

      <h3>Security Considerations</h3>
      <p>
        This tool processes all spreadsheet data locally in your browser. Whether your data comes from CSV files, Google
        Spreadsheets, or Excel, it never leaves your device, ensuring complete privacy and security. However, always be
        cautious when executing generated SQL on sensitive databases and verify the SQL before execution.
      </p>
    </ToolExplanation>
  )
}
...
function SqlConverterTool() {
  ...
  const convertToSql = () => {
    ...
    if ((outputFormat === "update" || outputFormat === "insert") && !tableName.trim()) {
      toast({
    ...
    try {
      ...
      if (outputFormat === "update") {
        ...
        const mappedHeaders =
          columnMappings.length === csvHeaders.length ? columnMappings : csvHeaders.map((header) => toSnakeCase(header))
        ...
        const finalCastings =
          columnCastings.length === csvHeaders.length ? columnCastings : new Array(csvHeaders.length).fill("")
    ...
    LOOP
        ...
        SET ${updateColumns
          .map((col, idx) => {
            ...
            const casting = finalCastings[castingIndex] ? `${finalCastings[castingIndex]}` : ""
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {(outputFormat === "update" || outputFormat === "insert") && headers.length > 0 && (
              <div className="space-y-2">
                <Label>Column Mapping & Casting</Label>
                <div className="border rounded-md p-4 space-y-2">
                  <div className="text-sm text-muted-foreground mb-2">
                    Map your CSV headers to database column names and specify optional type casting:
                  </div>
                  ...
                  {headers.map((header, index) => (
                    <div key={index} className="grid grid-cols-[1fr_1fr_1fr] gap-2 items-center">
                      ...
                      <div className="relative">
                        <input
                          ...
                          onChange={(e) => updateColumnCasting(index, e.target.value)}
                        />
