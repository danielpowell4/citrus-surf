...
export default function CustomDiffViewer({ left, right, showUnchanged }: DiffViewerProps) {
    ...
    const computeDiff = async () => {
      try {
        ...
        const calculateStats = (obj: any) => {
          ...
          const processValue = (value: any) => {
            if (Array.isArray(value) && value.length > 0) {
              ...
              else if (value.length === 2 && value[0] !== " " && value[1] !== " ") changed++
        ...
        if (delta) {
          ...
          setDiffHtml("<div class='text-center p-4'>No differences found</div>")
        }
    ...
    <div>
      <div className="mb-4 flex flex-row items-center justify-between">
        ...
        <div className="flex gap-2">
          <Tabs value={viewMode} onValueChange={(value) => setViewMode(value as "visual" | "raw")}>
          ...
          <Button variant="outline" size="sm" onClick={copyToClipboard} className="flex items-center gap-1 h-8">
            {copied ? <ClipboardCheck className="h-3 w-3" /> : <Clipboard className="h-3 w-3" />}
            <span className="text-xs">Copy</span>
          </Button>
          <Button variant="outline" size="sm" onClick={downloadDiff} className="flex items-center gap-1 h-8">
