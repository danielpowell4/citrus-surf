...
export const metadata: Metadata = {
  title: "Lookup Fields Roadmap - Citrus Surf",
  description: "Implementation roadmap for lookup fields feature",
};
...
function formatMarkdown(content: string): string {
  ...
  return content
    ...
    .replace(/^- \[ \] (.*$)/gm, '<li class="mb-1 flex items-start"><span class="inline-block w-4 h-4 mr-2 mt-0.5 border border-gray-300 rounded"></span> $1</li>')
    .replace(/^- \[x\] (.*$)/gm, '<li class="mb-1 flex items-start"><span class="inline-block w-4 h-4 mr-2 mt-0.5 bg-green-500 border border-green-500 rounded text-white text-xs leading-none text-center">✓</span> $1</li>')
    .replace(/^- (.*$)/gm, '<li class="mb-1">$1</li>')
    .replace(/(<li.*<\/li>)/s, '<ul class="list-disc list-inside mb-4 space-y-1 text-gray-700 dark:text-gray-300">$1</ul>')
...
export default function LookupFieldsRoadmap() {
  const content = readMarkdownFile("lookup-fields/roadmap.md");
